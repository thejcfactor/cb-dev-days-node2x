...
class Repository {
  constructor() {
    ...
    let { host, bucket, username, password } = global.configuration.database;
    this.connect(host, bucket, username, password);
  }

  connect(host, bucketName, username, password) {
    this.host = `couchbase://${host}`;
    this.bucketName = bucketName;
    this.username = username;
    this.password = password;

    this.cluster = new couchbase.Cluster(host);
  ...
  createAccount(userInfo, callback) {
    try{
      ...
      this.getNewCustomerDocument(userInfo, function(err, customerDoc){
        ...
        scope.bucket.insert(customerDoc._id, customerDoc, function(err, savedCustomer){
          if(err){
            return callback(err, null);
          }
